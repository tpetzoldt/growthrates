...
parametric fits with function `all_growthrates` need more care and a little bit 
...
In the following, let's fit a Baranyi growth model to the data [@Baranyi1995], a
...
many_baranyi1 <- all_growthmodels(value ~ time | strain + conc + replicate,
                      data = bactgrowth,
                      FUN = grow_baranyi, p = p, lower = lower, upper=upper, 
                      log="y", ncores = 2)
```
...
pp[,"h0"] <- 0.65
## re-fit models
many_baranyi2 <- all_growthmodels(value ~ time | strain + conc + replicate,
                      data = bactgrowth,
                      grow_baranyi, p=pp, which=c("y0", "mumax", "K"),
